...
```{r}
resOrdered <- red[order(red$pvalue),]
```
...
```{r}
sub_red <- subset(resOrdered, padj <0.01)

write.csv(as.data.frame(sub_red), file="/Users/Carolina/Documents/GitHub/GenomeAnalysis/results/10_deseq2/dev_deseq2_results_pval0.01.csv")
```
...
```{r}
gene2=which.min(red$padj)
p<- plotCounts(dds, gene2, intgroup="condition", returnData = TRUE)

ggplot(p, aes(x=condition, y=count)) + 
  geom_point(position=position_jitter(w=0.1,h=0)) + 
  scale_y_log10(breaks=c(100, 1000, 5000))+
  ggtitle(rownames(red)[gene2])+
...
### Plot PCA
```{r}
vsd_dev = varianceStabilizingTransformation(dds)
```
